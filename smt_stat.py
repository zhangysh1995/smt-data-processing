...
now = '../2017-07-18-python-15s/2017-07-18-'
...
def single_data(solver):
    data = np.genfromtxt(now + solver + '.csv', delimiter=',')
    return data
...
def time_distri(solver):
    data = single_data(solver)
    time = data[:, 1]
    distri = np.histogram(time[1:], bins=(0.0, 0.01, 0.02, 0.04, 0.06))
    plt.pie(distri[1]/sum(distri[1]), labels=[0.0, 0.01, 0.02, 0.04, 0.06])
    plt.legend()
    plt.show()
